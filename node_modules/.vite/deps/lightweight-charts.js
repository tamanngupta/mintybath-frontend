...
*/
var customStyleDefaults$1 = { color: "#2196f3" };
...
function getDashPattern(style, lineWidth) {
	switch (style) {
		case 0: return [];
		case 1: return [lineWidth, lineWidth];
		case 2: return [2 * lineWidth, 2 * lineWidth];
		case 3: return [6 * lineWidth, 6 * lineWidth];
		case 4: return [lineWidth, 4 * lineWidth];
		default: return [];
	}
}
...
var ColorParser = class {
	constructor(customParsers, initialCache) {
		...
		if (initialCache) this._private__rgbaCache = initialCache;
	}
	/**
	...
	*/
	...
	_private__parseColor(color) {
		...
		const match = getRgbStringViaBrowser(color).match(/^rgba?\s*\((\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d*\.?\d+))?\)$/);
		if (!match) {
			if (this._private__customParsers.length) for (const parser of this._private__customParsers) {
...
var Crosshair = class extends DataSource {
	...
	_internal_snapToVisibleSeriesIfNeeded(index) {
		if (!this._private__options.doNotSnapToHiddenSeriesIndices) return index;
		...
		for (const series of model._internal_visibleSerieses()) {
			const leftResult = series._internal_bars()._internal_search(index, -1);
			if (leftResult) {
				if (leftResult._internal_index === index) return index;
				if (closestLeftIndex === null || leftResult._internal_index > closestLeftIndex) closestLeftIndex = leftResult._internal_index;
			}
			const rightResult = series._internal_bars()._internal_search(index, 1);
			if (rightResult) {
				if (rightResult._internal_index === index) return index;
				if (closestRightIndex === null || rightResult._internal_index < closestRightIndex) closestRightIndex = rightResult._internal_index;
			}
		}
		...
		return candidates[distances.indexOf(Math.min(...distances))];
	}
...
var formatterOptions = { _internal_decimalSign: "." };
/**
...
var HorizontalLineRenderer = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal_hitTest(x, y) {
		if (!this._private__data?._internal_visible) return null;
...
var CONFLATION_ERROR_MESSAGES = { _internal_missingPriceValueBuilder: "Custom series with conflation reducer must have a priceValueBuilder method" };
...
var DataConflater = class {
	...
	*/
	_internal_updateLastConflatedChunk(originalData, newLastRow, conflationLevel, customReducer, isCustomSeries = false, priceValueBuilder) {
		if (conflationLevel < 1 || originalData.length === 0) return originalData;
		...
		if (!cachedRows) return this._internal_conflateByFactor(originalData, conflationLevel, customReducer, isCustomSeries, priceValueBuilder);
	...
	_private__normalizeConflationLevel(barsToMerge) {
		if (barsToMerge <= 2) return 2;
		for (const level of CONFLATION_LEVELS) if (barsToMerge <= level) return level;
	...
	*/
	_private__buildRecursively(data, targetLevel, customReducer, isCustomSeries = false, priceValueBuilder, levelResults = /* @__PURE__ */ new Map()) {
		if (targetLevel === 2) return this._private__buildLevelFromOriginal(data, 2, customReducer, isCustomSeries, priceValueBuilder);
	...
	*/
	...
	_private__buildChunksFromData(data, mergeFactor, customReducer, isCustomSeries = false, priceValueBuilder) {
		const chunks = [];
		for (let i = 0; i < data.length; i += mergeFactor) if (data.length - i >= mergeFactor) {
			...
			chunks.push(merged);
		} else if (chunks.length === 0) chunks.push(this._private__plotRowToChunk(data[i], true));
		else {
	...
	_private__mergeTwoRows(a, b, customReducer, isCustomSeries = false, priceValueBuilder) {
		if (!isCustomSeries || !customReducer || !priceValueBuilder) {
			const high = a._internal_value[1] > b._internal_value[1] ? a._internal_value[1] : b._internal_value[1];
			const low = a._internal_value[2] < b._internal_value[2] ? a._internal_value[2] : b._internal_value[2];
			return {
				...
				_internal_open: a._internal_value[0],
				_internal_high: high,
				_internal_low: low,
				_internal_close: b._internal_value[3],
		...
		const aggregated = customReducer(this._private__convertToContext(a, priceValueBuilder), this._private__convertToContext(b, priceValueBuilder));
		...
		return {
			...
			_internal_endTime: b._internal_time,
			_internal_open: a._internal_value[0],
			_internal_high: Math.max(a._internal_value[1], p),
			_internal_low: Math.min(a._internal_value[2], p),
			_internal_close: p,
			_internal_originalDataCount: this._private__sumCount(a._internal_originalDataCount, b._internal_originalDataCount),
	...
	_private__mergeChunkAndRow(chunk, row, customReducer, isCustomSeries = false, priceValueBuilder) {
		if (!isCustomSeries || !customReducer || !priceValueBuilder) return {
			...
			_internal_high: chunk._internal_high > row._internal_value[1] ? chunk._internal_high : row._internal_value[1],
			_internal_low: chunk._internal_low < row._internal_value[2] ? chunk._internal_low : row._internal_value[2],
			_internal_close: row._internal_value[3],
	...
	_private__mergeRangeWithOverride(data, start, end, overrideIndex, overrideRow, customReducer, isCustomSeries = false, priceValueBuilder) {
		...
		if (end - start === 1) return this._private__plotRowToChunk(first, true);
	...
	*/
	_private__updateLastChunkInCache(originalData, newLastRow, conflationLevel, cachedRows, isCustomSeries = false, customReducer, priceValueBuilder) {
		...
		if (Math.min(chunkStartIndex + conflationLevel, originalData.length) - chunkStartIndex < conflationLevel && originalData.length > conflationLevel) {
		...
		if (Math.floor((lastOriginalIndex - 1) / conflationLevel) === Math.floor(lastOriginalIndex / conflationLevel) || cachedRows.length === 1) {
			...
			if (count <= 0) return cachedRows;
			const mergedChunk = count === 1 ? this._private__plotRowToChunk(chunkStartIndex === lastOriginalIndex ? newLastRow : originalData[chunkStartIndex], true) : this._private__mergeRangeWithOverride(originalData, chunkStartIndex, actualEndIndex, lastOriginalIndex, newLastRow, customReducer, isCustomSeries, priceValueBuilder);
	...
	_private__plotRowToChunk(item, isRemainder = false) {
		return {
			...
			_internal_open: item._internal_value[0],
			_internal_high: item._internal_value[1],
			_internal_low: item._internal_value[2],
			_internal_close: item._internal_value[3],
...
var barStyleFnMap = {
	Bar: (findBar, barStyle, barIndex, precomputedBars) => {
		...
		return { _internal_barColor: currentBar._internal_color ?? (isUp ? upColor : downColor) };
	},
	...
	Custom: (findBar, customStyle, barIndex, precomputedBars) => {
		return { _internal_barColor: ensureNotNull(findBar(barIndex, precomputedBars))._internal_color ?? customStyle.color };
	},
	...
	Histogram: (findBar, histogramStyle, barIndex, precomputedBars) => {
		return { _internal_barColor: ensureNotNull(findBar(barIndex, precomputedBars))._internal_color ?? histogramStyle.color };
	}
};
...
var PrimitivePaneViewWrapper = class {
	...
	_internal_renderer() {
		...
		if (this._private__cache?._internal_base === baseRenderer) return this._private__cache._internal_wrapper;
...
var PrimitiveWrapper = class {
	...
	_internal_paneViews() {
		...
		if (this._private__paneViewsCache?._internal_base === base) return this._private__paneViewsCache._internal_wrapper;
...
var SeriesPrimitiveWrapper = class extends PrimitiveWrapper {
	...
	_internal_timeAxisViews() {
		...
		if (this._private__timeAxisViewsCache?._internal_base === base) return this._private__timeAxisViewsCache._internal_wrapper;
		const timeScale = this._private__series._internal_model()._internal_timeScale();
	...
	_internal_priceAxisViews() {
		...
		if (this._private__priceAxisViewsCache?._internal_base === base) return this._private__priceAxisViewsCache._internal_wrapper;
	...
	_internal_priceAxisPaneViews() {
		...
		if (this._private__priceAxisPaneViewsCache?._internal_base === base) return this._private__priceAxisPaneViewsCache._internal_wrapper;
	...
	_internal_timeAxisPaneViews() {
		...
		if (this._private__timeAxisPaneViewsCache?._internal_base === base) return this._private__timeAxisPaneViewsCache._internal_wrapper;
		const wrapper = base.map((pw) => new SeriesPrimitivePaneViewWrapper(pw));
...
var Series = class extends PriceDataSource {
	constructor(model, seriesType, options, createPaneView, customPaneView) {
		...
		if (lineBasedSeries.includes(this._private__seriesType)) this._private__lastPriceAnimationPaneView = new SeriesLastPriceAnimationPaneView(this);
		...
		if (this._private__seriesType === "Custom") {
			...
			if (paneView._internal_conflationReducer) this._internal_setCustomConflationReducer(paneView._internal_conflationReducer);
		}
	}
	_internal_destroy() {
	...
	_internal_applyOptions(options) {
		...
		if (priceScaleId !== void 0 && priceScaleId !== this._private__options.priceScaleId) model._internal_moveSeriesToScale(this, priceScaleId);
		if (visible !== void 0 && visible !== this._private__options.visible) model._internal_invalidateVisibleSeries();
	...
	_internal_setData(data, updateInfo) {
		...
		this._private__conflationByFactorCache.clear();
		const tsOptions = this._internal_model()._internal_timeScale()._internal_options();
		if (tsOptions.enableConflation && tsOptions.precomputeConflationOnInit) this._private__precomputeConflationLevels(tsOptions.precomputeConflationPriority);
	...
	*/
	_internal_isConflationEnabled() {
		if (!this._internal_model()._internal_timeScale()._internal_options().enableConflation) return false;
	...
	*/
	_internal_updateLastConflatedChunk(newLastRow) {
		if (!this._internal_isConflationEnabled()) return;
		const conflationFactor = this._private__calculateConflationFactor();
		if (!this._private__conflationByFactorCache.has(conflationFactor)) return;
	...
	_internal_conflatedBars() {
		const conflationEnabled = this._internal_model()._internal_timeScale()._internal_options().enableConflation;
		if (this._private__seriesType === "Custom" && this._private__customConflationReducer === null) return this._private__data;
		if (!conflationEnabled) return this._private__data;
		...
		return this._private__conflationByFactorCache.get(factor) ?? this._private__data;
	}
	...
	_internal_customSeriesPlotValuesBuilder() {
		if (this._private__seriesType !== "Custom") return;
		return (data) => {
	...
	_private__autoscaleInfoImpl(startTimePoint, endTimePoint) {
		...
		this._private__primitives.forEach((primitive) => {
			...
			if (primitiveAutoscale?.margins) margins = primitiveAutoscale.margins;
		});
	...
	_private__getConflationParams() {
		...
		return {
			...
			_internal_effectiveSmoothing: this._private__options.conflationThresholdFactor ?? globalSmoothing ?? 1
		};
	}
	_private__buildConflatedListByFactor(factor) {
		...
		if (this._private__seriesType === "Custom" && this._private__customConflationReducer !== null) {
			...
			if (!priceValueBuilder) throw new Error(CONFLATION_ERROR_MESSAGES._internal_missingPriceValueBuilder);
		...
		} else conflatedRows = this._private__dataConflater._internal_conflateByFactor(originalRows, factor);
	...
	_private__precomputeConflationLevels(priority) {
		if (this._private__seriesType === "Custom" && (this._private__customConflationReducer === null || !this._internal_customSeriesPlotValuesBuilder())) return;
		this._private__conflationByFactorCache.clear();
		...
		for (const lvl of conflateFactors) {
			...
			if (globalObj?._internal_scheduler?._internal_postTask) globalObj._internal_scheduler._internal_postTask(() => {
				task();
	...
	_private__precomputeConflationLevel(factor) {
		if (this._private__conflationByFactorCache.has(factor)) return;
		if (this._private__data._internal_rows().length === 0) return;
		const list = this._private__buildConflatedListByFactor(factor);
		this._private__conflationByFactorCache.set(factor, list);
	}
};
var magnetPlotRowKeys = [3];
...
function isBaseDecimal(value) {
	...
	if (value > 0xde0b6b3a7640000) return true;
...
function hoveredSourceOnTopOrder(sources, hoveredSource, enabled) {
	...
	if (hoveredIndex === -1 || hoveredIndex === sources.length - 1) return sources;
	const reorderedSources = [];
	for (let i = 0; i < sources.length; i++) if (i !== hoveredIndex) reorderedSources.push(sources[i]);
...
var PriceTickMarkBuilder = class {
	...
	_internal_rebuildTickMarks() {
		...
		for (let i = 0; i < this._private__marks.length; i++) this._private__marks[i]._internal_label = labels[i];
	}
	_internal_marks() {
	...
	_private__applyEdgeMarks(firstValue, span, minCoord, maxCoord, minPadding, maxPadding) {
		...
		if (marks.length > 0 && marks[0]._internal_coord - topMark._internal_coord < spanPx / 2) marks.shift();
		if (marks.length > 0 && bottomMark._internal_coord - marks[marks.length - 1]._internal_coord < spanPx / 2) marks.pop();
	...
	_private__computeBoundaryPriceMark(firstValue, coord, minPadding, maxPadding) {
		...
		const valueSpan = Math.max(.1, this._internal_tickSpan(maxValue, minValue));
	...
	_private__shouldApplyEdgeMarks(span, low, high) {
		...
		if (this._private__priceScale._internal_isLog()) range = convertPriceRangeFromLog(range, this._private__priceScale._internal_getLogFormula());
...
var PriceScale = class {
	...
	_internal_orderedSources() {
		if (!this._private__cachedOrderedSources) this._private__cachedOrderedSources = sortSources(this._private__dataSources);
	...
	_internal_formatLogicalTickmarks(logicals) {
		switch (this._private__options.mode) {
			case 2: return this._private__formatPercentageTickmarks(logicals);
			case 3: return this._internal_formatter().formatTickmarks(logicals);
			default: return this._private__formatTickmarks(logicals);
		}
	}
	...
	_internal_updateFormatter() {
		...
		let zOrder = Infinity;
		this._private__formatterSource = null;
		for (const source of this._private__dataSources) if (source._internal_zorder() < zOrder) {
			zOrder = source._internal_zorder();
			this._private__formatterSource = source;
		}
		let base = 100;
		if (this._private__formatterSource !== null) base = Math.round(this._private__formatterSource._internal_base());
		this._private__formatter = defaultPriceFormatter;
		if (this._internal_isPercentage()) {
			...
			base = 100;
		} else if (this._private__formatterSource !== null) this._private__formatter = this._private__formatterSource._internal_formatter();
	...
	_private__recalculatePriceRangeImpl() {
		if (this._internal_isCustomPriceRange() && !this._internal_isAutoScale()) return;
...
var Pane = class {
	...
	_internal_removeDataSource(source, keepSourceOrder) {
		...
		this._private__dataSources.splice(index, 1);
		if (!keepSourceOrder) this._private__dataSources.forEach((ds, i) => ds._internal_setZorder(i));
	...
	_internal_defaultPriceScale() {
		...
		let priceScale = null;
		if (primaryPriceScale._internal_options().visible && primaryPriceScale._internal_dataSources().length !== 0) priceScale = primaryPriceScale;
		else if (secondaryPriceScale._internal_options().visible && secondaryPriceScale._internal_dataSources().length !== 0) priceScale = secondaryPriceScale;
		else if (this._private__dataSources.length !== 0) priceScale = this._private__dataSources[0]._internal_priceScale();
		if (priceScale === null) priceScale = this._internal_defaultVisiblePriceScale() ?? primaryPriceScale;
		return priceScale;
	}
	_internal_defaultVisiblePriceScale() {
		const [primaryPriceScale, secondaryPriceScale] = this._private__defaultPriceScalePair();
		if (primaryPriceScale._internal_options().visible) return primaryPriceScale;
		if (secondaryPriceScale._internal_options().visible) return secondaryPriceScale;
		return null;
	}
	...
	_internal_orderedSourcesForRendering() {
		...
		if (cached !== null && cached._internal_base === base && cached._internal_hovered === hovered && cached._internal_enabled === enabled) return cached._internal_result;
	...
	_internal_detachPrimitive(source) {
		this._private__primitives = this._private__primitives.filter((wrapper) => wrapper._internal_primitive() !== source);
		if (source.detached) source.detached();
		this._private__model._internal_lightUpdate();
	}
	...
	_private__createPriceScale(id, options) {
		const priceScale = new PriceScale(id, {
			visible: true,
			autoScale: true,
			...clone(options)
		}, this._private__model._internal_options()["layout"], this._private__model._internal_options().localization, this._private__model._internal_colorParser());
...
function isBetterHit(candidate, currentBest) {
	...
	if (candidate._internal_priority === 2 && currentBest._internal_priority !== 2) return true;
	if (currentBest._internal_priority === 2 && candidate._internal_priority !== 2) return false;
	if (candidate._internal_distance !== currentBest._internal_distance) return candidate._internal_distance < currentBest._internal_distance;
	return false;
}
...
function findBestPrimitiveHitTest(sources, x, y) {
	...
	if (!bestPrimitiveHit || !bestHitSource || !bestPrimitiveCandidate) return null;
	return {
...
function hitTestPaneView(paneViews, x, y, pane) {
	...
	for (const paneView of paneViews) {
		...
		if (candidate !== null) {
			...
			if (bestResult === null || isBetterHit(candidateResult._internal_candidate, bestResult._internal_candidate)) bestResult = candidateResult;
		}
	}
	return bestResult;
}
...
function hitTestPane(pane, x, y) {
	...
	if (bestPrimitiveHit?._internal_hit.zOrder === "top") return convertPrimitiveHitResult(bestPrimitiveHit);
	let bestSourceHit = null;
	let bestSourceCandidate = null;
	for (const source of sources) {
		if (bestPrimitiveHit && bestPrimitiveHit._internal_source === source && bestPrimitiveHit._internal_hit.zOrder !== "bottom" && !bestPrimitiveHit._internal_hit.isBackground) return bestSourceHit ?? convertPrimitiveHitResult(bestPrimitiveHit);
	...
	if (bestSourceHit !== null) return bestSourceHit;
	if (bestPrimitiveHit?._internal_hit) return convertPrimitiveHitResult(bestPrimitiveHit);
	return null;
}
...
var TickMarks = class {
	...
	_internal_build(spacing, maxWidth, checkIndicesForData, indicesWithDataMap, indicesWithDataId) {
		...
		if (this._private__cache === null || this._private__cache._internal_maxIndexesPerMark !== maxIndexesPerMark || indicesWithDataId !== this._private__cache._internal_indicesWithDataId || checkIndicesForData !== this._private__cache._internal_checkIndicesForData) this._private__cache = {
	...
	_private__buildMarksImpl(maxIndexesPerMark, checkIndicesForData, indicesWithDataMap) {
		...
		for (const weight of Array.from(this._private__marksByWeight.keys()).sort((a, b) => b - a)) {
			...
			for (; prevMarksPointer < prevMarksLength; prevMarksPointer++) if (canBeIncluded(prevMarks[prevMarksPointer])) marks.push(prevMarks[prevMarksPointer]);
		}
		return marks;
	}
};
...
var TimeScale = class {
	...
	_internal_applyOptions(options, localizationOptions) {
		...
		this._private__checkRightOffsetPixels(options);
		if (options.minBarSpacing !== void 0 || options.maxBarSpacing !== void 0) this._private__model._internal_setBarSpacing(options.barSpacing ?? this._private__barSpacing);
		if (options.ignoreWhitespaceIndices !== void 0 && options.ignoreWhitespaceIndices !== this._private__options.ignoreWhitespaceIndices) this._internal_recalculateIndicesWithData();
		this._private__invalidateTickMarks();
		this._private__updateDateTimeFormatter();
		if (options.enableConflation !== void 0 || options.conflationThresholdFactor !== void 0) this._private__updateConflationFactor();
	...
	_internal_coordinateToIndex(x, considerIgnoreWhitespace) {
		...
		if (!considerIgnoreWhitespace || !this._private__options.ignoreWhitespaceIndices || this._private__shouldConsiderIndex(index)) return index;
	...
	_internal_restoreDefault() {
		...
		let newOffset;
		if (this._private__options.rightOffsetPixels !== void 0) newOffset = this._private__options.rightOffsetPixels / this._internal_barSpacing();
		else newOffset = this._private__options.rightOffset;
	...
	*/
	...
	_internal_setVisibleRange(range, applyDefaultOffset) {
		...
		if (applyDefaultOffset) this._private__rightOffset = pixelOffset ? pixelOffset / this._internal_barSpacing() : this._private__options.rightOffset;
		this._private__correctOffset();
		this._private__visibleRangeInvalidated = true;
		this._private__model._internal_recalculateAllPanes();
		this._private__model._internal_lightUpdate();
	...
	_internal_recalculateIndicesWithData() {
		if (!this._private__options.ignoreWhitespaceIndices) return;
		this._private__indicesWithData.clear();
		const series = this._private__model._internal_serieses();
		for (const s of series) for (const index of s._internal_fulfilledIndices()) this._private__indicesWithData.set(index, true);
	...
	*/
	_internal_possibleConflationFactors() {
		const conflationThreshold = 1 / (window.devicePixelRatio || 1);
		const minBarSpacing = this._private__options.minBarSpacing;
		if (minBarSpacing >= conflationThreshold) return [1];
		const factors = [1];
		let currentLevel = 2;
		const maxLevel = 512;
		while (currentLevel <= maxLevel) {
			if (minBarSpacing < conflationThreshold / currentLevel) factors.push(currentLevel);
			currentLevel *= 2;
		}
		return factors;
	}
	...
	_private__maxBarSpacing() {
		if (this._private__options.maxBarSpacing > 0) return this._private__options.maxBarSpacing;
		else return this._private__width * .5;
	}
	...
	*/
	...
	_private__shouldConsiderIndex(index) {
		if (!this._private__options.ignoreWhitespaceIndices) return true;
	...
	_private__findNearestIndexWithData(x) {
		...
		while (maxIndex) {
			...
			if (this._private__indicesWithData.get(index)) return index;
...
var ChartModel = class {
	...
	_internal_setHoveredSource(source) {
		if (this._private__hoveredSource?._internal_source === source?._internal_source && this._private__hoveredSource?._internal_object?._internal_externalId === source?._internal_object?._internal_externalId && this._private__hoveredSource?._internal_object?._internal_hitTestData === source?._internal_object?._internal_hitTestData && this._private__hoveredSource?._internal_cursorStyle === source?._internal_cursorStyle && this._private__hoveredSource?._internal_itemType === source?._internal_itemType) return;
		...
		if (source !== null && source._internal_source !== prevSource?._internal_source) this._internal_updateSource(source._internal_source);
	}
	_internal_options() {
		return this._private__options;
	}
	...
	_internal_applyPriceScaleOptions(priceScaleId, options, paneIndex = 0) {
		...
		if (pane === void 0) throw new Error(`Trying to apply price scale options with incorrect pane index: ${paneIndex}`);
		if (priceScaleId === "left") {
			merge(this._private__options, { leftPriceScale: options });
			pane._internal_applyScaleOptions({ leftPriceScale: options });
			this._private__priceScalesOptionsChanged._internal_fire();
			this._internal_fullUpdate();
			return;
		} else if (priceScaleId === "right") {
			merge(this._private__options, { rightPriceScale: options });
			pane._internal_applyScaleOptions({ rightPriceScale: options });
			this._private__priceScalesOptionsChanged._internal_fire();
			this._internal_fullUpdate();
			return;
		}
	...
	_internal_removePane(index) {
		if (this._private__panes.length === 1) return;
	...
	_internal_changePanesHeight(paneIndex, height) {
		if (this._private__panes.length < 2) return;
		...
		for (const pane of this._private__panes) if (pane !== targetPane) {
	...
	*/
	...
	_internal_visibleSerieses() {
		if (this._private__visibleSerieses === null) this._private__visibleSerieses = this._private__serieses.filter((s) => s._internal_visible());
	...
	_internal_paneForSource(source) {
		if (isPanePrimitive(source)) return source;
	...
	_internal_defaultVisiblePriceScaleId() {
		...
		if (leftVisible !== this._private__options.rightPriceScale.visible) return leftVisible ? "left" : "right";
	...
	_internal_moveSeriesToPane(series, newPaneIndex) {
		...
		if (newPaneIndex === this._private__seriesPaneIndex(series)) return;
		...
		if (previousPane._internal_dataSources().length === 0) paneWasRemoved = this._private__cleanupIfPaneIsEmpty(previousPane);
		if (!paneWasRemoved) this._internal_fullUpdate();
	}
	...
	_private__getOrCreatePane(index) {
		...
		if (index < this._private__panes.length) return this._private__panes[index];
	...
	_private__addSeriesToPane(series, pane) {
		...
		if (!isDefaultPriceScale(targetScaleId)) series._internal_applyOptions(series._internal_options());
	}
...
function isWindows() {
	if (!isRunningOnClientSide) return false;
	if (navigator?.userAgentData?.platform) return navigator.userAgentData.platform === "Windows";
...
function hoveredTargetSourceKind(source, itemType) {
	if (source instanceof Pane) return "pane-primitive";
	if (itemType === "marker" || itemType === "primitive") return "series-primitive";
	return "series";
}
function hoveredTargetObjectKind(itemType, objectId) {
	switch (itemType) {
		case "custom": return objectId !== void 0 ? "custom-object" : "series";
...
function buildHoveredEventInfo(hoveredSource, paneIndex) {
	...
	if (hoveredSource === null || hoveredSource._internal_itemType === void 0) return {
...
var PaneSeparator = class {
	constructor(chartWidget, topPaneIndex, bottomPaneIndex) {
		...
		if (!this._private__resizeEnabled) {
			this._private__handle = null;
			this._private__mouseEventHandler = null;
		} else this._private__addResizableHandle();
	}
	_internal_destroy() {
		if (this._private__mouseEventHandler !== null) this._private__mouseEventHandler._internal_destroy();
	}
	...
	_internal_update() {
		...
		if (this._private__chartWidget._internal_options()["layout"].panes.enableResize !== this._private__resizeEnabled) {
			this._private__resizeEnabled = this._private__chartWidget._internal_options()["layout"].panes.enableResize;
			if (this._private__resizeEnabled) this._private__addResizableHandle();
			else {
	...
	_private__mouseOverEvent(event) {
		if (this._private__handle !== null) this._private__handle._internal_element.style.backgroundColor = this._private__chartWidget._internal_options()["layout"].panes.separatorHoverColor;
	}
	_private__mouseLeaveEvent(event) {
		if (this._private__handle !== null && this._private__resizeInfo === null) this._private__handle._internal_element.style.backgroundColor = "";
	}
	_private__mouseDownEvent(event) {
		if (this._private__handle === null) return;
		...
		const pixelStretchFactor = totalStretch / (this._private__topPane._internal_getSize().height + this._private__bottomPane._internal_getSize().height);
		const minPaneStretch = 30 * pixelStretchFactor;
		if (totalStretch <= minPaneStretch * 2) return;
	...
	_private__pressedMouseMoveEvent(event) {
		...
		if (resizeInfo === null) return;
		const deltaStretchFactor = (event.pageY - resizeInfo._internal_startY) * resizeInfo._internal_pixelStretchFactor;
	...
	_private__mouseUpEvent(event) {
		if (this._private__resizeInfo === null || this._private__handle === null) return;
		this._private__resizeInfo = null;
...
function createBoundCanvas(parentElement, size) {
	...
	const binding = bindTo(canvas, {
		...
		options: { allowResizeObserver: true },
...
function buildPriceAxisViewsGetter(zOrder, priceScaleId) {
	return (source) => {
		if (!hasPriceScale(source)) return [];
		if ((source._internal_priceScale()?._internal_id() ?? "") !== priceScaleId) return [];
...
function recalculateOverlapping(views, direction, scaleHeight, rendererOptions) {
	...
	for (let i = 1; i < views.length; i++) {
		...
		if (direction === 1 ? coordinate > prevCoordinate - height : coordinate < prevCoordinate + height) {
			...
			if ((direction === 1 ? edgePoint < 0 : edgePoint > scaleHeight) && spaceBeforeCurrentGroup > 0) {
				...
				for (let k = currentGroupStart; k < views.length; k++) views[k]._internal_setRenderCoordinate(views[k]._internal_getRenderCoordinate() + direction * possibleShift);
...
var PriceAxisWidget = class {
	...
	_internal_optimalWidth() {
		...
		const ctx = ensureNotNull(this._private__canvasBinding.canvasElement.getContext("2d", { colorSpace: this._private__pane._internal_chart()._internal_options().layout.colorSpace }));
		ctx.save();
	...
	_internal_paint(type) {
		...
		const canvasOptions = { colorSpace: this._private__pane._internal_chart()._internal_options().layout.colorSpace };
		if (type !== 1) {
	...
	_private__alignLabels() {
		...
		const updateForSources = (sources) => {
			sources.forEach((source) => {
				...
				if (centerSource === source && sourceViews.length > 0) center = sourceViews[0]._internal_coordinate();
			});
		};
	...
	_private__fixLabelOverlap(views, rendererOptions, center) {
		...
		if (top.length && bottom.length) bottom.push(top[0]);
		...
		for (const view of views) {
			...
			if (coordinate > -halfHeight && coordinate < halfHeight) view._internal_setRenderCoordinate(halfHeight);
			if (coordinate > this._private__size.height - halfHeight && coordinate < this._private__size.height + halfHeight) view._internal_setRenderCoordinate(this._private__size.height - halfHeight);
		}
...
var PaneWidget = class PaneWidget {
	...
	_internal_recalculatePriceScales() {
		...
		for (const primitive of pane._internal_primitives()) primitive._internal_updateAllViews();
	}
	_internal_getBitmapSize() {
		return this._private__canvasBinding.bitmapSize;
	}
	_internal_drawBitmap(ctx, x, y, addTopLayer) {
		...
		if (bitmapSize.width > 0 && bitmapSize.height > 0) {
			...
			if (addTopLayer) {
				const topLayer = this._private__topCanvasBinding.canvasElement;
				if (ctx !== null) ctx.drawImage(topLayer, x, y);
			}
		}
	}
	_internal_paint(type) {
		...
		const canvasOptions = { colorSpace: this._private__chart._internal_options().layout.colorSpace };
		if (type !== 1) {
			this._private__canvasBinding.applySuggestedBitmapSize();
			const target = tryCreateCanvasRenderingTarget2D(this._private__canvasBinding, canvasOptions);
			if (target !== null) {
	...
	_private__drawGrid(target) {
		...
		const renderer = state._internal_grid()._internal_paneView()._internal_renderer(state);
	...
	_private__drawSources(target, paneViewsGetter) {
		...
		for (const panePrimitive of panePrimitives) this._private__drawSourceImpl(target, paneViewsGetter, drawBackground, panePrimitive);
		for (const source of sources) this._private__drawSourceImpl(target, paneViewsGetter, drawBackground, source);
		for (const panePrimitive of panePrimitives) this._private__drawSourceImpl(target, paneViewsGetter, drawForeground, panePrimitive);
...
var PriceAxisStub = class {
	...
	_internal_paint(type) {
		...
		this._private__invalidated = false;
		this._private__canvasBinding.applySuggestedBitmapSize();
		const target = tryCreateCanvasRenderingTarget2D(this._private__canvasBinding, { colorSpace: this._private__options.layout.colorSpace });
...
var TimeAxisWidget = class {
	...
	_internal_paint(type) {
		if (type === 0) return;
		const canvasOptions = { colorSpace: this._private__options.colorSpace };
		if (type !== 1) {
			this._private__canvasBinding.applySuggestedBitmapSize();
			const target = tryCreateCanvasRenderingTarget2D(this._private__canvasBinding, canvasOptions);
			if (target !== null) {
	...
	_private__drawAdditionalSources(target, axisViewsGetter) {
		...
		for (const source of sources) drawSourceViews(axisViewsGetter, (renderer) => drawBackground(renderer, target, false, void 0), source, void 0);
		for (const source of sources) drawSourceViews(axisViewsGetter, (renderer) => drawForeground(renderer, target, false, void 0), source, void 0);
	}
	_private__drawBackground({ context: ctx, bitmapSize }) {
...
var ChartWidget = class {
	...
	_internal_destroy() {
		...
		this._private__paneWidgets = [];
		for (const paneSeparator of this._private__paneSeparators) this._private__destroySeparator(paneSeparator);
		this._private__paneSeparators = [];
	...
	_internal_applyOptions(options) {
		...
		if (options["layout"]?.panes) this._private__applyPanesOptions();
		this._private__updateTimeAxisVisibility();
	...
	*/
	_private__traverseLayout(ctx, addTopLayer) {
		...
		const drawPriceAxises = (position, targetX) => {
			...
			for (let paneIndex = 0; paneIndex < this._private__paneWidgets.length; paneIndex++) {
				...
				if (ctx !== null) priceAxisWidget._internal_drawBitmap(ctx, targetX, targetY, addTopLayer);
				...
				if (paneIndex < this._private__paneWidgets.length - 1) {
					...
					if (ctx !== null) separator._internal_drawBitmap(ctx, targetX, targetY);
		...
		for (let paneIndex = 0; paneIndex < this._private__paneWidgets.length; paneIndex++) {
			...
			if (ctx !== null) paneWidget._internal_drawBitmap(ctx, totalWidth, totalHeight, addTopLayer);
			totalHeight += bitmapSize.height;
			if (paneIndex < this._private__paneWidgets.length - 1) {
				const separator = this._private__paneSeparators[paneIndex];
				const separatorBitmapSize = separator._internal_getBitmapSize();
				if (ctx !== null) separator._internal_drawBitmap(ctx, totalWidth, totalHeight);
	...
	_private__adjustSizeImpl() {
		...
		const separatorsHeight = 1 * this._private__paneSeparators.length;
		...
		for (let paneIndex = 0; paneIndex < this._private__paneWidgets.length; ++paneIndex) {
			...
			if (paneIndex === this._private__paneWidgets.length - 1) calculatePaneHeight = Math.ceil((totalPaneHeight - accumulatedHeight) * pixelRatio) / pixelRatio;
			else calculatePaneHeight = Math.round(paneWidget._internal_stretchFactor() * stretchPixels * pixelRatio) / pixelRatio;
	...
	_private__syncGuiWithModel() {
		...
		for (let i = targetPaneWidgetsCount; i < actualPaneWidgetsCount; i++) {
			...
			if (paneSeparator !== void 0) this._private__destroySeparator(paneSeparator);
		}
	...
	_private__getPaneIndex(pane) {
		let paneIndex = -1;
		if (pane) paneIndex = this._private__paneWidgets.indexOf(pane);
		else {
			const crosshairPane = this._internal_model()._internal_crosshairSource()._internal_pane();
			if (crosshairPane !== null) paneIndex = this._internal_model()._internal_panes().indexOf(crosshairPane);
		}
		return paneIndex;
	}
...
function seriesUpdateInfo(seriesRows, prevSeriesRows, bh) {
	...
	if (firstAndLastTime !== void 0 && prevFirstAndLastTime !== void 0) return {
...
var DataLayer = class {
	...
	_internal_updateSeriesData(series, data, historicalUpdate) {
		if (historicalUpdate && series._internal_isConflationEnabled()) throw new Error("Historical updates are not supported when conflation is enabled. Conflation requires data to be processed in order.");
		...
		if (!historicalUpdate && lastSeriesTime !== void 0 && this._private__horzScaleBehavior.key(time) < this._private__horzScaleBehavior.key(lastSeriesTime)) throw new Error(`Cannot update oldest data, last time=${lastSeriesTime}, new time=${time}`);
		let pointDataAtTime = this._private__pointDataByTimePoint.get(this._private__horzScaleBehavior.key(time));
		if (historicalUpdate && pointDataAtTime === void 0) throw new Error("Cannot update non-existing data point when historicalUpdate is true");
		...
		if (historicalUpdate) this._private__updateHistoricalSeriesRow(series, plotRow, pointDataAtTime._internal_index);
		...
		} else this._private__updateLastSeriesRow(series, plotRow);
		const info = {
	...
	_internal_popSeriesData(series, count) {
		...
		if (seriesData === void 0 || count <= 0) return [[], this._private__emptyUpdateResponse()];
		count = Math.min(count, seriesData.length);
		const poppedData = seriesData.splice(-count).reverse();
		if (seriesData.length === 0) this._private__seriesLastTimePoint.delete(series);
		else this._private__seriesLastTimePoint.set(series, seriesData[seriesData.length - 1]._internal_time);
		for (const data of poppedData) {
			...
			if (pointData._internal_mapping.size !== 0) continue;
			...
			for (let index = pointData._internal_index; index < this._private__sortedTimePoints.length; ++index) assignIndexToPointData(this._private__sortedTimePoints[index].pointData, index);
		}
		return [poppedData, this._private__getUpdateResponse(series, this._private__sortedTimePoints.length - 1, {
			_internal_historicalUpdate: false,
	...
	_private__updateHistoricalSeriesRow(series, plotRow, pointDataIndex) {
		...
		if (isSeriesPlotRow(plotRow)) seriesData[index] = plotRow;
		else seriesData.splice(index, 1);
	}
...
function distanceToSegment(x, y, x1, y1, x2, y2) {
	const deltaX = x2 - x1;
	const deltaY = y2 - y1;
	if (deltaX === 0 && deltaY === 0) return Math.hypot(x - x1, y - y1);
...
function slotStart(item, previousItem, barSpacing) {
	if (previousItem === void 0 || previousItem._internal_time !== item._internal_time - 1) return item._internal_x - barSpacing / 2;
	return (previousItem._internal_x + item._internal_x) / 2;
}
function slotEnd(item, nextItem, barSpacing) {
	if (nextItem === void 0 || nextItem._internal_time !== item._internal_time + 1) return item._internal_x + barSpacing / 2;
...
function hitTestSeriesRange(items, visibleRange, x, y, barSpacing, hitTestTolerance, rangeProvider) {
	if (visibleRange === null || visibleRange.from >= visibleRange.to || items.length === 0) return null;
	...
	if (candidateFrom >= candidateTo) return null;
...
var SeriesPaneViewBase = class {
	...
	_internal_hitTest(x, y) {
		if (!this._internal__series._internal_visible()) return null;
		this._internal__makeValid();
		if (this._internal__itemsVisibleRange === null) return null;
	...
	_internal__makeValid() {
		...
		const currentConflationKey = timeScale._internal_options().enableConflation ? timeScale._internal_conflationFactor() : 0;
...
var SeriesCustomPaneView = class extends SeriesPaneViewBase {
	...
	_internal__hitTestImpl(x, y) {
		...
		if (customHit !== null && customHit !== void 0) return normalizeCustomHit(customHit);
		const fallbackHit = hitTestSeriesRange(this._internal__items, this._internal__itemsVisibleRange, x, y, this._internal__model._internal_timeScale()._internal_barSpacing(), this._internal__series._internal_options().hitTestTolerance, (bar, out) => {
			...
			let top = NaN;
			let bottom = NaN;
			if (originalData !== void 0 && !this._private__paneView.isWhitespace(originalData)) for (const price of this._private__paneView.priceValueBuilder(originalData)) {
...
var createPaneView$6 = (series, model, customPaneView) => {
	return new SeriesCustomPaneView(series, model, ensure(customPaneView));
};
...
var PaneApi = class {
	...
	moveTo(paneIndex) {
		...
		if (currentIndex === paneIndex) return;
	...
	getHTMLElement() {
		...
		if (!widgets || widgets.length === 0 || !widgets[this.paneIndex()]) return null;
	...
	priceScale(priceScaleId) {
		if (this._private__pane._internal_priceScaleById(priceScaleId) === null) throw new Error(`Cannot find price scale with id: ${priceScaleId}`);
...
function getChecker(type) {
	switch (type) {
		...
		case "Custom": return checkCustomItem.bind(null);
	}
}
function checkBarItem(type, barItem) {
	if (!isFulfilledData(barItem)) return;
	[
		"open",
		"high",
		"low",
...
function checkLineItem(type, lineItem) {
	if (!isFulfilledData(lineItem)) return;
...
var SeriesApi = class {
	...
	pop(count = 1) {
		...
		if (poppedRows.length !== 0) this._private__onDataChanged("update");
	...
	lastValueData(globalLast) {
		...
		if (result._internal_noData) return { noData: true };
		return {
			noData: false,
...
var ChartApi = class {
	...
	addCustomSeries(customPaneView, options = {}, paneIndex = 0) {
		const definition = createCustomSeriesDefinition(ensure(customPaneView));
	...
	_internal_popData(series, count) {
		...
		if (poppedData.length !== 0) this._private__sendUpdateToChart(update);
		return poppedData;
	}
	...
	applyOptions(options) {
		{
			...
			if (colorSpace !== void 0 && colorSpace !== this.options().layout.colorSpace) throw new Error(`colorSpace option should not be changed once the chart has been created.`);
			const colorParsers = options.layout?.colorParsers;
			if (colorParsers !== void 0 && colorParsers !== this.options().layout.colorParsers) throw new Error(`colorParsers option should not be changed once the chart has been created.`);
		}
	...
	takeScreenshot(addTopLayer = false, includeCrosshair = false) {
		...
		try {
			if (!includeCrosshair) {
				...
				this._private__chartWidget._internal_applyOptions({ crosshair: { mode: 2 } });
			}
			screenshotCanvas = this._private__chartWidget._internal_takeScreenshot(addTopLayer);
		} finally {
			if (!includeCrosshair && crosshairMode !== void 0) this._private__chartWidget._internal_model()._internal_applyOptions({ crosshair: { mode: crosshairMode } });
		}
		return screenshotCanvas;
	}
	...
	_private__addSeriesImpl(definition, options = {}, paneIndex = 0) {
		...
		if (definition.type === "Candlestick") fillUpDownCandlesticksColors(options);
...
function createChartEx(container, horzScaleBehavior, options) {
	const res = new ChartApi(fetchHtmlElement(container), horzScaleBehavior, options);
...
function drawSeriesPointMarkers(renderingScope, items, pointMarkersRadius, visibleRange, styleGetter) {
	if (visibleRange.to - visibleRange.from <= 0) return;
...
function walkLine(renderingScope, items, lineType, visibleRange, barWidth, styleGetter, finishStyledArea, dashPatternLength = 0) {
	...
	if (visibleRange.to - visibleRange.from < 2) {
		...
		for (let i = visibleRange.from + 1; i < visibleRange.to; ++i) {
			...
			switch (lineType) {
				...
				case 1: {
					...
					if (shouldTrackDashOffset) accumulatedDistance += Math.abs(currentItem._internal_x - prevItem._internal_x) * horizontalPixelRatio;
					if (itemStyle !== currentStyle) {
						changeStyle(itemStyle, currentItem);
						ctx.lineTo(currentX, prevY);
					}
					ctx.lineTo(currentX, currentY);
					if (shouldTrackDashOffset) accumulatedDistance += Math.abs(currentItem._internal_y - prevItem._internal_y) * verticalPixelRatio;
					break;
				}
				case 2: {
		...
		if (shouldTrackDashOffset) ctx.lineDashOffset = 0;
	}
}
var curveTension = 6;
...
var PaneRendererLineBase = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal__drawImpl(renderingScope) {
		...
		if (lineType !== void 0) walkLine(renderingScope, items, lineType, visibleRange, barWidth, styleGetter, finishStyledArea$1, dashPatternLength);
...
function lineSegmentHorizontalBounds(firstItem, secondItem, lineType, items, toItemIndex) {
	switch (lineType) {
		case 2: {
			...
			return [Math.min(firstItem._internal_x, secondItem._internal_x, firstControlPoint._internal_x, secondControlPoint._internal_x), Math.max(firstItem._internal_x, secondItem._internal_x, firstControlPoint._internal_x, secondControlPoint._internal_x)];
		}
		default: return [Math.min(firstItem._internal_x, secondItem._internal_x), Math.max(firstItem._internal_x, secondItem._internal_x)];
	}
}
...
function hitTestLineSeries(items, visibleRange, x, y, lineType, lineWidth, pointMarkersRadius, barSpacing = 0, hitTestTolerance = 0) {
	...
	if (pointMarkersRadius !== void 0) {
		...
		for (let itemIndex = pointCandidateFrom; itemIndex < pointCandidateTo; itemIndex++) {
			const item = items[itemIndex];
			if (!isWithinHorizontalSweep(x, item._internal_x, item._internal_x, pointMarkersRadius + hitTestTolerance)) continue;
			const distance = Math.hypot(x - item._internal_x, y - item._internal_y);
			if (distance <= pointMarkersRadius + hitTestTolerance) pointMinDistance = Math.min(pointMinDistance, distance);
		}
	}
	if (visibleRange.to - visibleRange.from < 2) {
		...
		if (distance <= radius) pointMinDistance = Math.min(pointMinDistance, distance);
	...
	for (let itemIndex = segmentFrom; itemIndex < segmentTo; itemIndex++) {
		...
		if (!isWithinHorizontalSweep(x, leftX, rightX, radius)) continue;
		const distance = hitTestLineSegment(x, y, previousItem, currentItem, lineType, items, itemIndex, radius);
		if (distance !== null) lineMinDistance = Math.min(lineMinDistance, distance);
	}
	if (Number.isFinite(pointMinDistance)) return hoveredSeriesHitTestResult(pointMinDistance, 2, "series-point");
...
var LinePaneViewBase = class extends SeriesPaneViewBase {
	...
	_internal__fillRawPoints() {
		...
		this._internal__items = this._internal__series._internal_conflatedBars()._internal_rows().map((row) => {
			...
			let value;
			if (isConflated) {
				const high = row._internal_value[1];
				const low = row._internal_value[2];
				const close = row._internal_value[3];
				value = Math.abs(high - close) > Math.abs(low - close) ? high : low;
			} else value = row._internal_value[3];
...
var YieldCurveHorzScaleBehavior = class {
	...
	createConverterToInternalObj(data) {
		...
		return (time) => {
			if (time > this._private__largestIndex) this._private__largestIndex = time;
	...
	fillWeightsForPoints(sortedTimePoints, startIndex) {
		const timeWeight = (time) => {
			if (time % 120 === 0) return 10;
			if (time % 60 === 0) return 9;
			if (time % 36 === 0) return 8;
			if (time % 12 === 0) return 7;
			if (time % 6 === 0) return 6;
			if (time % 3 === 0) return 5;
			if (time % 1 === 0) return 4;
			return 0;
		};
		for (let index = startIndex; index < sortedTimePoints.length; ++index) sortedTimePoints[index].timeWeight = timeWeight(fromInternalHorzScaleItem(sortedTimePoints[index].time));
	...
	_private__formatTime(months) {
		if (this._private__options.localization?.timeFormatter) return this._private__options.localization.timeFormatter(months);
...
function generateWhitespaceData({ _internal_start: start, _internal_end: end, _internal_resolution: resolution }) {
	return Array.from({ length: Math.floor((end - start) / resolution) + 1 }, (item, i) => ({ "time": start + i * resolution }));
}
...
var defaultOptions = {
	...
	timeScale: { ignoreWhitespaceIndices: true },
	leftPriceScale: { visible: true },
	rightPriceScale: { visible: false },
	localization: { priceFormatter: (value) => {
...
var YieldChartApi = class extends ChartApi {
	...
	addSeries(definition, options = {}, paneIndex = 0) {
		if (definition.isBuiltIn && ["Area", "Line"].includes(definition.type) === false) throw new Error("Yield curve only support Area and Line series");
...
function createYieldCurveChart(container, options) {
	return new YieldChartApi(fetchHtmlElement(container), options);
}
...
var HorzScaleBehaviorPrice = class {
	...
	fillWeightsForPoints(sortedTimePoints, startIndex) {
		const priceWeight = (price) => {
			if (price === Math.ceil(price / 100) * 100) return 8;
			if (price === Math.ceil(price / 50) * 50) return 7;
			if (price === Math.ceil(price / 25) * 25) return 6;
			if (price === Math.ceil(price / 10) * 10) return 5;
			if (price === Math.ceil(price / 5) * 5) return 4;
			if (price === Math.ceil(price)) return 3;
			if (price * 2 === Math.ceil(price * 2)) return 1;
			return 0;
		};
		for (let index = startIndex; index < sortedTimePoints.length; ++index) sortedTimePoints[index].timeWeight = priceWeight(sortedTimePoints[index].time);
	}
	_private__precision() {
...
var SeriesBaselinePaneView = class extends LineHitTestPaneViewBase {
	...
	_internal__prepareRendererData() {
		...
		if (this._internal__itemsVisibleRange === null || this._internal__items.length === 0) return;
		let topCoordinate;
		...
		if (options.relativeGradient) {
			...
			for (let i = this._internal__itemsVisibleRange.from; i < this._internal__itemsVisibleRange.to; i++) {
				...
				if (item._internal_y < topCoordinate) topCoordinate = item._internal_y;
				if (item._internal_y > bottomCoordinate) bottomCoordinate = item._internal_y;
			}
		}
...
var SeriesHistogramPaneView = class extends LinePaneViewBase {
	...
	_internal__hitTestImpl(x, y) {
		...
		if (histogramBase === null) return null;
...
var PanePrimitiveWrapper = class {
...
var TextWatermarkRenderer = class {
	...
	draw(target) {
		target.useMediaCoordinateSpace((scope) => {
			if (!this._private__data.visible) return;
...
var TextWatermark = class {
	...
	_internal_applyOptions(options) {
		...
		if (this._internal_requestUpdate) this._internal_requestUpdate();
	}
};
/**
...
var ImageWatermarkRenderer = class {
	...
	draw(target) {
		target.useMediaCoordinateSpace((scope) => {
			...
			if (!pos || !this._private__data._internal_imgElement) return;
	...
	_private__determinePlacement(data, paneSize) {
		...
		if (maxHeight) availableHeight = Math.min(availableHeight, maxHeight);
		if (maxWidth) availableWidth = Math.min(availableWidth, maxWidth);
		...
		return {
			_internal_x: plotCentreX - .5 * drawWidth,
			_internal_y: plotCentreY - .5 * drawHeight,
...
var ImageWatermarkPaneView = class {
	...
	_internal_stateUpdate(state) {
		if (state._internal_imageWidth !== void 0) this._private__imageWidth = state._internal_imageWidth;
		if (state._internal_imageHeight !== void 0) this._private__imageHeight = state._internal_imageHeight;
		if (state._internal_image !== void 0) this._private__image = state._internal_image;
		this._internal_update();
	}
...
var ImageWatermark = class {
	...
	attached(attachedParams) {
		...
		this._private__imgElement.onload = () => {
			...
			if (this._private__requestUpdate) this._private__requestUpdate();
		};
...
var SeriesPrimitiveAdapter = class {
	...
	applyOptions(options) {
		if (this._internal__primitive && this._internal__primitive._internal_applyOptions) this._internal__primitive._internal_applyOptions(options);
	}
	_private__attach() {
...
function hitTestArrow(up, centerX, centerY, size, x, y) {
	const halfArrowSize = (shapeSize("arrowUp", size) - 1) / 2;
	const halfBaseSize = (ceiledOdd(size / 2) - 1) / 2;
	...
	if (x >= baseLeft && x <= baseRight && y >= baseTop - rectTolerance && y <= baseBottom + rectTolerance) return true;
	const isInTriangleBounds = () => {
		...
		if (x < headLeft || x > headRight || y < headTop || y > headBottom) return false;
		const dx = Math.abs(x - centerX);
		return (up ? Math.abs(y - centerY) : Math.abs(y - centerY)) + triangleTolerance >= dx / 2;
	};
...
var SeriesMarkersRenderer = class {
	...
	_internal_hitTest(x, y) {
		...
		for (let i = this._private__data._internal_visibleRange.from; i < this._private__data._internal_visibleRange.to; i++) {
			const item = this._private__data._internal_items[i];
			if (item && hitTestItem(item, x, y)) return {
				zOrder: "normal",
	...
	draw(target) {
		if (this._private__zOrder === "aboveSeries") return;
		target.useBitmapCoordinateSpace((scope) => {
...
function getPrice(seriesData, marker, isInverted) {
	if (isPriceMarker(marker.position) && marker.price !== void 0) return marker.price;
	if (isValueData(seriesData)) return seriesData.value;
	if (isOhlcData(seriesData)) {
		if (marker.position === "inBar") return seriesData.close;
		if (marker.position === "aboveBar") {
			if (!isInverted) return seriesData.high;
			return seriesData.low;
		}
		if (marker.position === "belowBar") {
			if (!isInverted) return seriesData.low;
...
function fillSizeAndY(rendererItem, marker, seriesData, offsets, textHeight, shapeMargin, series, chart) {
	...
	switch (marker.position) {
		...
		case "atPriceTop": {
			...
			if (!ignoreOffset) offsets._internal_aboveBar += shapeSize + shapeMargin;
			return;
		}
		case "belowBar":
		case "atPriceBottom": {
			...
			if (!ignoreOffset) offsets._internal_belowBar += shapeSize + shapeMargin;
			return;
		}
	}
}
...
var SeriesMarkersPaneView = class {
	...
	renderer() {
		if (!this._private__series.options().visible) return null;
	...
	_internal_update(updateType) {
		this._private__invalidated = true;
		if (updateType === "data") this._private__dataInvalidated = true;
	}
	...
	_internal__makeValid() {
		...
		if (this._private__series.data()[0] === null) return;
		...
		for (let index = this._private__data._internal_visibleRange.from; index < this._private__data._internal_visibleRange.to; index++) {
			...
			const dataAt = this._private__series.dataByIndex(marker.time, 0);
...
var SeriesMarkersPrimitive = class {
	...
	detached() {
		if (this._private__series && this._private__dataChangedHandler) this._private__series.unsubscribeDataChanged(this._private__dataChangedHandler);
		this._private__chart = null;
		this._private__series = null;
		this._private__paneView = null;
		this._private__dataChangedHandler = null;
	...
	hitTest(x, y) {
		if (this._private__paneView) return this._private__paneView.renderer()?._internal_hitTest(x, y) ?? null;
		return null;
	}
	...
	_private__getAutoScaleMargins() {
		const barSpacing = ensureNotNull(this._private__chart).timeScale().options().barSpacing;
	...
	_private__getMarkerPositions() {
		if (this._private__markersPositions === null) this._private__markersPositions = this._private__markers.reduce((acc, marker) => {
	...
	_private__recalculateMarkers() {
		if (!this._private__recalculationRequired || !this._private__chart || !this._private__series) return;
		const timeScale = this._private__chart.timeScale();
		...
		this._private__indexedMarkers = this._private__markers.map((marker, index) => {
			...
			const baseMarker = {
				time: timeScale.timeToIndex(ensureNotNull(seriesDataByIndex).time, false),
			...
			if (marker.position === "atPriceTop" || marker.position === "atPriceBottom" || marker.position === "atPriceMiddle") {
				if (marker.price === void 0) throw new Error(`Price is required for position ${marker.position}`);
				return {
					...baseMarker,
					position: marker.position,
					price: marker.price
...
function createSeriesMarkers(series, markers, options) {
	...
	if (markers) wrapper.setMarkers(markers);
	return wrapper;
}
var ExpiringMarkerManager = class {
	...
	_internal_setMarker(marker, key, timeout) {
		...
		} else this._private__markers.set(key, {
			...marker,
	...
	_internal_clearMarker(key) {
		...
		if (marker && marker._internal_timeoutId !== void 0) window.clearTimeout(marker._internal_timeoutId);
		this._private__markers.delete(key);
		this._private__triggerUpdate();
	}
	_internal_clearAllMarkers() {
		for (const [point] of this._private__markers) this._internal_clearMarker(point);
	}
	_internal_getMarkers() {
		const now = Date.now();
		const activeMarkers = [];
		for (const [time, marker] of this._private__markers) if (!marker._internal_expiresAt || marker._internal_expiresAt > now) activeMarkers.push({
			time: marker.time,
			sign: marker.sign,
			value: marker.value
		});
		else this._internal_clearMarker(time);
	...
	_private__triggerUpdate() {
		if (this._private__updateCallback) this._private__updateCallback();
	}
};
...
var MarkersPrimitiveRenderer = class {
	...
	draw(target) {
		target.useBitmapCoordinateSpace((scope) => {
			const ctx = scope.context;
			const correction = Math.max(1, Math.floor(scope.horizontalPixelRatio)) % 2 / 2;
			...
			this._private__data.forEach((item) => {
				...
				if (item._internal_sign) {
					...
					ctx.lineTo(item._internal_x * scope.horizontalPixelRatio + correction, (item._internal_y - 7 * item._internal_sign - 7 * item._internal_sign * .5) * scope.verticalPixelRatio);
	...
	_private__getColor(sign) {
		if (sign === 0) return this._private__neutralColor;
...
function getNeutralColor(opts, seriesType) {
	if (isAreaStyleOptions(opts, seriesType)) return opts.lineColor;
	return opts.color;
}
var MarkersPrimitivePaneView = class {
	...
	_internal_update(markers) {
		this._private__data = markers.map((marker) => {
			...
			return {
				_internal_x: ensureNotNull(this._private__timeScale.timeToCoordinate(marker.time)),
				_internal_y: y,
	...
	renderer() {
		const neutralColor = getNeutralColor(this._private__series.options(), this._private__series.seriesType());
...
var UpDownMarkersPrimitive = class {
	...
	attached(params) {
		...
		if (seriesType !== "Area" && seriesType !== "Line") throw new Error("UpDownMarkersPrimitive is only supported for Area and Line series types");
	...
	_internal_setData(data) {
		if (!this._private__series) throw new Error("Primitive not attached to series");
		const seriesType = this._private__series.seriesType();
		this._private__managedDataPoints.clear();
		const horzBehaviour = this._private__horzScaleBehavior;
		if (horzBehaviour) data.forEach((d) => {
			if (isFulfilledData(d) && isLineData(d, seriesType)) this._private__managedDataPoints.set(horzBehaviour.key(d.time), d.value);
		});
		ensureDefined(this._private__series).setData(data);
	}
	_internal_update(data, historicalUpdate) {
		if (!this._private__series || !this._private__horzScaleBehavior) throw new Error("Primitive not attached to series");
		const seriesType = this._private__series.seriesType();
		const horzKey = this._private__horzScaleBehavior.key(data.time);
		if (isWhitespaceData(data)) this._private__managedDataPoints.delete(horzKey);
		if (isFulfilledData(data) && isLineData(data, seriesType)) {
			const existingPrice = this._private__managedDataPoints.get(horzKey);
			if (existingPrice) this._private__markersManager._internal_setMarker({
				time: data.time,
				value: data.value,
...
function createUpDownMarkers(series, options = {}) {
	return new SeriesUpDownMarkerPrimitiveWrapper(series, new UpDownMarkersPrimitive(options));
}
